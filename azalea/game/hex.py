...
class HexGameState:
    def __init__(self, color: int, legal_moves: np.ndarray, result: int, board: np.ndarray):
        self.color = color  # 0=first player (red), 1=second player (blue)
...
        """
    ...
    @staticmethod
    def flip_player_board_moves(board: np.ndarray, moves: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if len(board.shape) == 2:
            ...
            return HexGame.flip_player_board_moves(board[None, :, :], moves[None, :])
        ...
        tiles = np.ravel_multi_index((board_size[1] - 1 - tile_ids[1], board_size[0] - 1 - tile_ids[0]),
                                     board_size)
        flipped_moves = np.zeros_like(moves).ravel
        flipped_moves[mask] = tiles.astype(np.int32) + 1
        flipped_moves = flipped_moves.reshape(moves.shape)
...
class HexGameImpl:
    ...
    def legal_moves(self):
        ...
        moves = [i + 1 for i in range(self.board.size) if self.board.flat[i] == 0]
    ...
    def result(self):
        """Has game terminated."""
        if self.winner:
            return 1 if self.winner == 2 else 3
        return 0
...
def neighbors(tile, size):
    ...
    n = 0
    for ni, nj in [(ti - 1, tj), (ti - 1, tj + 1), (ti, tj - 1),
                   (ti, tj + 1), (ti + 1, tj - 1), (ti + 1, tj)]:
...
if __name__ == '__main__':
    ...
    for g in range(100):
        ...
        for m in range(1000):
            ...
            print(f'Move {m} {["x", "o"][game.state.color]}: '
                  f'{game.io.format_move(game.state.board, move)}')
            game.step(move)
